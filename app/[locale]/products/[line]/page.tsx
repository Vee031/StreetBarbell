...
export default async function ProductLinePage({ params }: { params: Promise<{ locale: string; line: string }> }) {
  ...
  const lineProducts = await getMergedProductsByLine(line);
  return <><section className="page-hero product-line-hero" style={{ backgroundImage: `linear-gradient(90deg,rgba(8,11,16,.92),rgba(8,11,16,.45)),url('${productLine.image}')` }}><div className="page-shell"><Link className="back-link" href={`/${locale}/products`}><ArrowLeft size={16}/>{d.products.back}</Link><span className="eyebrow light">{d.nav.products}</span><h1>{locale === "cs" ? productLine.nameCs : productLine.nameEn}</h1><p>{productLine.count} {d.products.machines}</p></div></section><section className="section page-shell">{lineProducts.length ? <div className="product-grid">{lineProducts.map((product) => <ProductCard key={product.code} product={product} locale={locale} t={d.products}/>)}</div> : <p>{d.products.noProducts}</p>}</section></>;
}
